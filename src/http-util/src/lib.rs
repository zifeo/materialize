...
//! ## Proxy exclusions
//!
//! The `no_proxy` environment variable specifies a comma-separated list of
//! addresses to exclude from proxying. It takes precedence over the other
...
//!   * an IP address prefix in CIDR notation (e.g., `1.2.3.4/16`), or
//!   * a domain name followed by an optional port (e.g., `foo.com`).
//!
//! Whitespace surrounding an entry is ignored. Note that `no_proxy` matching
//! never involves DNS resolution, so a `no_proxy` value of `1.2.3.4` will
//! exclude requests that literally mention the IP in the URL (e.g.,
//! `http://1.2.3.4`) from proxying, but not requests to
