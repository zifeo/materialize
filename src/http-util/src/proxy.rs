            ...
            _ => {
                ...
                for host in s.split(',') {
                    ...
                    if let Ok(net) = IpNet::from_str(host) {
                        ips.push(IpMatcher { net, port: None });
                        continue;
                    }
                    ...
                    if let Ok(addr) = IpAddr::from_str(host) {
                        ...
                        if !domain.starts_with('.') {
                            // See comment in `DomainMatcher::matches`.
                            domain.insert(0, '.')
                        }
                        hosts.push(DomainMatcher { domain, port })
                    }
                }
...
impl IpMatcher {
    fn matches(&self, ip: &IpAddr, port: u16) -> bool {
        self.net.contains(ip) && (self.port.is_none() || self.port == Some(port))
    }
}
...
impl DomainMatcher {
    fn matches(&self, host: &str, port: u16) -> bool {
        ...
        // `.materialize.com`, so that we can match `re.materialize.com` but not
        // `rematerialize.com`.
